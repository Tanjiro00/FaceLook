...
/** Helper: run fal.subscribe with a timeout */
async function falSubscribeWithTimeout<T>(
  model: string,
  ...
  const result = await Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error(`fal.ai timeout after ${timeoutMs / 1000}s on ${model}`)), timeoutMs)
    ),
  ]);
...
async function editWithNanoBanana(
  ...
  console.log("[pipeline] Stage 3: Nano Banana edit starting...");
  const data = await falSubscribeWithTimeout<unknown>("fal-ai/nano-banana/edit", {
    image_urls: [imageUrl],
    prompt,
    output_format: "jpeg",
    aspect_ratio: "auto",
  ...
  const parsed = nanoBananaResponseSchema.safeParse(data);
  if (!parsed.success) {
    console.error("[pipeline] Nano Banana unexpected response:", JSON.stringify(data).slice(0, 500));
...
async function restoreFace(imageUrl: string): Promise<string> {
  console.log("[pipeline] Stage 4: CodeFormer starting...");
  const data = await falSubscribeWithTimeout<unknown>("fal-ai/codeformer", {
    image_url: imageUrl,
    fidelity: 0.9,
  });

  const parsed = singleImageResponseSchema.safeParse(data);
  const url = parsed.success && parsed.data.image?.url ? parsed.data.image.url : imageUrl;
  console.log("[pipeline] Stage 4: done");
  return url;
}
...
async function upscaleHD(imageUrl: string): Promise<string> {
  console.log("[pipeline] Stage 5: Real-ESRGAN starting...");
  const data = await falSubscribeWithTimeout<unknown>("fal-ai/real-esrgan", {
    image_url: imageUrl,
    scale: 2,
  });
  ...
  const parsed = singleImageResponseSchema.safeParse(data);
  const url = parsed.success && parsed.data.image?.url ? parsed.data.image.url : imageUrl;
  console.log("[pipeline] Stage 5: done");
  return url;
}
...
export async function runPipeline(input: PipelineInput): Promise<PipelineResult> {
  ...
  console.log("[pipeline] Starting:", { procedureId: input.procedureId, intensity, hdUpscale: input.hdUpscale });

  // Stage 2: Build prompt
  const prompt = buildPrompt(input.procedureId, intensity);
  console.log("[pipeline] Stage 2: Prompt built");
